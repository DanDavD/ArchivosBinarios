...
public class EmpleadosManager{
    ...
    public EmpleadosManager(){
        try{
            ...
            //3.Inicializar el archivo de codigo si es nuestro
            initCode(); 
        }catch(IOException e) { 
    ...
    private void createEmployeeFolders(int code) throws IOException{
        ...
        edir.mkdirs();  
        //crear los archivos de la ventas
    ...
    /*
    Imprime:
    Realizar una lista de empleados No despedidos con la siguientes estructuras: 
    Codigo=Nombre - Salario - Fecha Contratatacion
    */ 
            remps.seek(0);
            while(remps.getFilePointer()<remps.length()){
            String empleado=remps.readInt()+" - "+remps.readUTF()+" - "+remps.readDouble()+"$ - "+new Date(remps.readLong());
    ...
    public void addSaleToEmployee(int code, double sale) throws IOException{
        ...
        salesFile.writeDouble(newSales);
            ...
            JOptionPane.showMessageDialog(null, "No se pudo porque el codigo no existe o fue inabilitado");
        }
